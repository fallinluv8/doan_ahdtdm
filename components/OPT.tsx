    ...
    for (let i = 0; i < referenceArray.length; i++) {
      ...
      if (!memoryFrames.includes(currentPage)) {
        ...
        if (memoryFrames.includes(-1)) {
          ...
          const futureReferences = referenceArray.slice(i + 1);
          const replaceIndex = memoryFrames
            .map((page) => {
              const futureIndex = futureReferences.indexOf(page);
              return futureIndex === -1 ? Infinity : futureIndex;
            })
            .indexOf(
              Math.max(
                ...memoryFrames.map((page) => {
                  const futureIndex = futureReferences.indexOf(page);
                  return futureIndex === -1 ? Infinity : futureIndex;
                })
              )
          ...
          memoryFrames[replaceIndex] = currentPage;
        }
      }
  ...
  return (
    <div className='flex flex-col justify-center md:items-center items-start mx-4'>
      ...
      <p className='text-xs md:text-xl lg:mx-40 mx-4 mb-10 dark:text-neutral-500 text-black'>
        Trong thuật toán này, các trang sẽ được thay thế mà không được sử dụng
        trong thời gian dài nhất trong tương lai.
      </p>
